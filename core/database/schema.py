...
class Urls(Base):
    ...
    last_accessed_at = Column(DateTime, nullable=False, server_default=func.now(), onupdate=func.now())

    # Relationships
    processed_contents = relationship("ProcessedMarkdownContent", back_populates="url_source")
    code_blocks = relationship("CodeBlock", back_populates="url_source")
    analysis_results = relationship("SecurityAnalysisResult", back_populates="url_source")
    analysis_history_entries = relationship("AnalysisHistory", back_populates="url_source")
...
class ProcessedMarkdownContent(Base):
    ...
    extracted_code_blocks = relationship("CodeBlock", back_populates="processed_content_source")
    extracted_items = relationship("ExtractedItems", back_populates="processed_content_source")
...
class SecurityAnalysisResult(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    url_id = Column(Integer, ForeignKey("urls.id"), nullable=False) # References the new Urls table
...
class CodeBlock(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    url_id = Column(Integer, ForeignKey("urls.id"), nullable=False) # Original source URL
    processed_content_id = Column(Integer, ForeignKey("processed_markdown_content.id"), nullable=False) # Link to specific parse
    ...
    processed_content_source = relationship("ProcessedMarkdownContent", back_populates="extracted_code_blocks")
...
class ExtractedItems(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    processed_content_id = Column(Integer, ForeignKey("processed_markdown_content.id"), nullable=False)
    item_type = Column(String(50), nullable=False)  # e.g., 'url', 'image_src', 'footnote_ref'
    target_value = Column(Text, nullable=False)    # The actual URL string or reference key
    source_text = Column(Text, nullable=True)      # e.g., anchor text for a link

    processed_content_source = relationship("ProcessedMarkdownContent", back_populates="extracted_items")
    __table_args__ = (Index("idx_processed_content_item_type", "processed_content_id", "item_type"),)
