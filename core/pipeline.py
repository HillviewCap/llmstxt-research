...
class Pipeline:
    def __init__(self, config=None):
        ...
        static_analyzer_config = {
            "max_content_size": self.config.get(
                "max_content_size", 1024 * 1024
            ),  # 1MB default
            "max_content_lines": self.config.get(
    ...
    def _init_monitoring(self):
        ...
        try:
            ...
            self.alert_manager.add_semgrep_resource_alert(
                max_processes=3, max_memory_percent=8.0
            )

            # Start monitoring in background with increased intervals
            self.metrics_collector.start_collection(
                interval=60
            )  # Check every 60 seconds (was 30)
            self.alert_manager.start_monitoring(
                interval=120
            )  # Check alerts every 120 seconds (was 60)

            self.logger.info(
                "Semgrep process monitoring initialized with optimized intervals"
            )
        ...
        try:
            ...
            try:
                ...
                if not content_items:
                    self.logger.warning(
                        "No content items retrieved. Pipeline will not proceed further."
                    )
                    return None
            except Exception as e:
            ...
            try:
                processed_items = [
                    self.content_processor.process_pipeline_item(item)
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    ...
                    for batch_start in range(0, len(processed_items), batch_size):
                        ...
                        for future in concurrent.futures.as_completed(
                            list(future_to_item_idx.keys())[-len(batch_items) :]
                        ):
                            ...
                            try:
                                ...
                                if isinstance(result, dict) and "error" in result:
                                    item_id = getattr(
                                        future, "item_id", f"item-{item_idx}"
                                    )
                                    self.logger.error(
                                        f"Analysis for item {item_id} failed: {result['error']}"
                                    )
                                ...
                                self.logger.error(
                                    f"Analysis for item {item_id} generated an exception: {exc}",
                                    exc_info=True,
                                )
                                analysis_results[item_idx] = {
                                    "error": str(exc)
                                }  # Store error info
            ...
            try:
                ...
                valid_analysis_results = [
                    res for res in analysis_results if "error" not in res
                ]
                if not valid_analysis_results:
                    self.logger.warning("No valid analysis results to score.")
                else:
                    scores = [
                        self.scoring_model.score(result)
                ...
                self.logger.error(
                    f"Scoring and risk assessment failed: {e}", exc_info=True
                )
                raise
            ...
            try:
                ...
                for result in analysis_results:
                    ...
                    for analyzer_type, analyzer_result in result.items():
                        if (
                            isinstance(analyzer_result, dict)
                            and "findings" in analyzer_result
                        ):
                ...
                for batch_start in range(0, len(content_items), batch_size):
                    ...
                    for i, item in enumerate(batch_items, batch_start):
                        if (
                            ...
                            temporal_result = self.temporal_analyzer.process_content(
                                url, content, processed_id
                            )
                            batch_results.append(
                                (i, url, temporal_result, valid_analysis_results[i])
                            )
                    ...
                    with self.db.session_scope() as session:
                        for i, url, temporal_result, analysis_result in batch_results:
                            ...
                            self.temporal_analyzer.track_analysis_result(
                                url, analysis_result, session=session
                            )
            ...
            self.logger.error(
                f"Pipeline execution failed catastrophically: {e}", exc_info=True
            )
            ...
            self.logger.info(
                f"Pipeline ran for {total_time:.2f}s before critical failure."
            )
    ...
    def _analyze_item(self, item):
        ...
        try:
            # Set database session for static analyzer if needed
            with self.db.session_scope() as session:
                # Set the database session for the static analyzer
                self.static_analyzer.set_db_session(session)
    ...
    def _calculate_timeout(self, item):
        ...
        for pattern in complex_patterns:
            ...
            complexity_factor += min(
                pattern_count, 10
            )  # Up to 10 seconds per pattern type
        ...
        self.logger.info(
            f"Calculated timeout for item {item_id}: {timeout:.1f}s (size: {content_size/1024:.1f}KB, lines: {line_count})"
        )
    ...
    def _execute_with_thread_timeout(self, func, args_tuple, timeout_seconds):
        ...
        def target_wrapper():
            try:
                ...
                error_container[0] = e
                self.logger.error(
                    f"Error in analysis thread for item {item_id}: {e}", exc_info=True
                )
            finally:
                completed_event.set()
        ...
        if completed:
            self.logger.info(
                f"Analysis for item {item_id} completed in {execution_time:.2f}s (memory: {memory_delta:.2f}MB)"
            )
            ...
            self.logger.error(
                f"Analysis thread timed out after {timeout_seconds} seconds for item {item_id}"
            )
            ...
            try:
                ...
                if frame:
                    ...
                    self.logger.error(
                        f"Thread stack trace at timeout for item {item_id}:\n{stack_trace}"
                    )
            except Exception as e:
    ...
    def _terminate_child_processes(self):
        ...
        for child in children:
            try:
                ...
                if (
                    "semgrep" in child.name().lower()
                    or "python" in child.name().lower()
                ):
                    self.logger.warning(
                        f"Terminating potentially hung child process: {child.pid} ({child.name()})"
                    )
                    ...
                    if still_alive:
                        self.logger.warning(
                            f"Force killing process {child.pid} that didn't terminate gracefully"
                        )
                        child.kill()
            except (
