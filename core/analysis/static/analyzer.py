...
class StaticAnalyzer:
    def __init__(
        self, rules_path: str = "rules/semgrep", config: Optional[Dict[str, Any]] = None
    ):
        ...
        self.max_content_lines = self.config.get(
    ...
    def analyze_code_blocks(self, processed_content_id: int) -> List[Dict[str, Any]]:
        ...
        if not self.db_session:
            raise StaticAnalyzerError(
                "Database session not set. Call set_db_session() first."
            )

        code_blocks = get_code_blocks_by_content_id(
            self.db_session, processed_content_id
        )
        if not code_blocks:
            logger.warning(
                f"No code blocks found for content ID: {processed_content_id}"
            )
            return []
    ...
    def analyze_single_block(self, block: CodeBlock) -> List[Dict[str, Any]]:
        ...
        if not block.content or not block.language:
            logger.warning(
                f"Skipping code block {block.id}: Missing content or language"
            )
            return []

        # Run semgrep on the block
        findings = self.semgrep_runner.run(
            content=block.content, language=block.language
        )
    ...
    def analyze(
        self, data: Any, language: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        import time
        import psutil
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                processed_content_id = data.get("processed_content_id", item_id)
                ...
                # Fetch code blocks from the database using the processed_content_id
                code_blocks = get_code_blocks_by_content_id(
                    self.db_session, processed_content_id
                )

                if not code_blocks:
                    # Fallback to content parsing if no code blocks found in the database
                    content_to_scan = data.get("content")
                    if not content_to_scan:
                        raise StaticAnalyzerError(
                            f"Input dictionary for analysis is missing 'content' key and no code blocks found in database. ID: {item_id}"
                        )
                    ...
                    parsed_blocks = parsed_data.get("code_blocks", [])

                    # Log code block analysis
                    logger.info(
                        f"Found {len(parsed_blocks)} code blocks to analyze from content parsing"
                    )
                    ...
                    for i, block in enumerate(parsed_blocks, 1):
                        ...
                        if code_content and block_language:
                            logger.info(
                                f"Analyzing code block {i}/{len(parsed_blocks)} with language {block_language}"
                            )

                            # Skip analysis for unsupported languages
                            if block_language.lower() not in [
                                lang.lower()
                                for lang in self.semgrep_runner.SUPPORTED_LANGUAGES
                            ]:
                                logger.info(
                                    f"Skipping unsupported language: {block_language}"
                                )
                                continue

                            # Run semgrep on the block
                            block_findings = self.semgrep_runner.run(
                                content=code_content, language=block_language
                            )
                    ...
                    content_lines = (
                        content_to_scan.count("\n") + 1 if content_to_scan else 0
                    )
                    print(
                        f"Item {item_id}: Content size: {content_size} bytes, {content_lines} lines"
                    )

                    # Check content size before processing
                    if content_to_scan and content_size > self.max_content_size:
                        print(
                            f"WARNING: Item {item_id}: Content size ({content_size} bytes) exceeds maximum allowed size ({self.max_content_size} bytes)"
                        )
                        return [
                            self._create_size_limit_finding(
                                item_id, content_size, self.max_content_size
                            )
                        ]
                    ...
                    if content_to_scan and content_lines > self.max_content_lines:
                        print(
                            f"WARNING: Item {item_id}: Content line count ({content_lines}) exceeds maximum allowed lines ({self.max_content_lines})"
                        )
                        return [
                            self._create_line_limit_finding(
                                item_id, content_lines, self.max_content_lines
                            )
                        ]
                else:
                    # Use the pre-processed code blocks from the database
                    logger.info(
                        f"Found {len(code_blocks)} code blocks in database for content ID: {processed_content_id}"
                    )

                    # Analyze each code block from the database
                    for i, block in enumerate(code_blocks, 1):
                        if block.content and block.language:
                            logger.info(
                                f"Analyzing code block {i}/{len(code_blocks)} with language {block.language}"
                            )

                            # Skip analysis for unsupported languages
                            if block.language.lower() not in [
                                lang.lower()
                                for lang in self.semgrep_runner.SUPPORTED_LANGUAGES
                            ]:
                                logger.info(
                                    f"Skipping unsupported language: {block.language}"
                                )
                                continue
                            ...
                            # Adjust line numbers based on the block's line_start
                            for finding in block_findings:
                                if (
                                    finding.get("start", {}).get("line")
                                    and block.line_start is not None
                                ):
                                    finding["start"]["line"] += block.line_start - 1
                                if (
                                    finding.get("end", {}).get("line")
                                    and block.line_start is not None
                                ):
                                    finding["end"]["line"] += block.line_start - 1
                ...
                target_path = data
                logger.info(
                    f"Static analyzing file: {target_path} (lang hint: {language})"
                )
                ...
                # Run semgrep
                findings = self.semgrep_runner.run(
                    target_path=target_path, language=language
                )
                ...
                raise StaticAnalyzerError(
                    f"Unsupported data type for analysis: {type(data)}. Must be dict or str."
                )
            ...
            print(
                f"Static analysis completed in {execution_time:.2f}s, memory delta: {memory_used:.2f}MB"
            )
            ...
            logger.error(
                f"Static analysis failed after {execution_time:.2f}s, memory delta: {memory_used:.2f}MB"
            )
            ...
            logger.exception(
                f"Unexpected error during static analysis ({error_type}): {e}"
            )
    ...
    def _log_semgrep_processes(self, stage):
        ...
        try:
            ...
            for proc in psutil.process_iter(
                ...
                try:
                    ...
                    if proc.info["name"] == "semgrep" or (
                        proc.info["cmdline"]
                        and any("semgrep" in cmd for cmd in proc.info["cmdline"] if cmd)
                    ):
                        ...
                        proc_info = {
                            ...
                            or proc.cpu_percent(interval=0.1),
                            "cmdline": (
            ...
            if semgrep_processes:
                ...
                long_running = [
                    p for p in semgrep_processes if p["running_time"] > 60
                ]  # > 1 minute
                if long_running:
    ...
    def _analyze_complex_generic_content(
        ...
        patterns = [
            ...
            (
                r'(password|api[_\s]?key|secret|token)[=:]\s*[\'"][^\'"]+[\'"]',
                "potential_secret",
            ...
            (
                r"(eval\(|exec\(|system\()",
                "dangerous_function",
                "Potentially dangerous function call",
                "High",
            ),
            (
                r"(DROP\s+TABLE|DELETE\s+FROM|UPDATE\s+.*\s+SET)",
                "sql_command",
                "SQL command detected",
                "Medium",
            ),
            (
                r"<script[^>]*>.*?<\/script>",
        ...
        for pattern, rule_id, message, priority in patterns:
            for match in re.finditer(
                ...
                if line_start == line_end:
                    col_start = (
                        start_pos - content[:start_pos].rfind("\n")
                        if "\n" in content[:start_pos]
                        else start_pos + 1
                    )
                    col_end = (
                        end_pos - content[:end_pos].rfind("\n")
                        if "\n" in content[:end_pos]
                        else end_pos + 1
                    )
                else:
        ...
        findings.append(
            {
                ...
                "end": {"line": 1, "col": 1},
                "extra": {
                    "message": "Complex generic content analyzed with alternative method instead of semgrep"
                },
                "category": "Info",
                "priority": "Low",
            }
    ...
    def _create_size_limit_finding(
        self, item_id: str, content_size: int, max_size: int
    ) -> Dict[str, Any]:
    ...
    def _create_line_limit_finding(
        self, item_id: str, line_count: int, max_lines: int
    ) -> Dict[str, Any]:
