...
class ContentRetriever:
    ...
    def retrieve(self, query=None):
        ...
        # For URL or file queries, try to use the query as the source
        if query and isinstance(query, str):
            # Handle local file paths (with or without file:// prefix)
            if query.startswith('file://'):
                filepath = query[7:]  # Remove file:// prefix
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        content = f.read()
                    return [self._create_content_item(filepath, content, "local_file")]
                except Exception as e:
            ...
            # Handle HTTP URLs
                ...
                try:
                    ...
                    return [self._create_content_item(query, content, "direct_url_query")]
                except ContentRetrievalError as e:
                    print(f"Error retrieving content from URL {query}: {e}")
                    return []
            
            print(f"Unsupported query type or file not found: {query}")
            return []

        # If no query is provided, fetch all URLs from the database
    ...
    def _create_content_item(self, source: str, content: str, retrieval_method: str) -> Dict[str, Any]:
        """Helper method to create a standardized content item."""
        return {
            "id": f"content-{hashlib.md5(source.encode()).hexdigest()[:8]}",
            "type": "markdown",
            "source": source,
            "language": "markdown",
            "content": content,
