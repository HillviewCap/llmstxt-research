    ...
    try:
        ...
        with open(config_path, 'r') as f:
            if ext.lower() == '.yaml' or ext.lower() == '.yml':
                ...
                logging.warning(f"Unknown config file type: {ext}. Attempting to parse as YAML.")
                config = yaml.safe_load(f)
                
        logging.info(f"Loaded configuration from {config_path}")
    except FileNotFoundError:
        logging.error(f"Configuration file not found: {config_path}. Using default empty config.")
    except (yaml.YAMLError, json.JSONDecodeError) as e:
        logging.error(f"Error parsing configuration file {config_path}: {e}. Using default empty config.")
    
    # Ensure db path is set, even if config loading fails or is partial
    if "db" not in config or "path" not in config.get("db", {}):
        logging.warning("Database path not found in config, using default: researchdb/llms_metadata.db")
...
def setup_sandbox(config):
    ...
    logging.info(f"Setting up sandbox environment in {sandbox_config.get('mode', 'mock')} mode")
...
def main():
    ...
    if args.mode == "analysis":
        ...
        logging.warning("Reporting mode currently generates a placeholder report. For full report, run with --mode all.")
        report_data = pipeline.reporting_manager.generate_report([], [], [], []) # Placeholder
        ...
        if active_alerts:
            for alert in active_alerts:
                print(f"  {alert['alert_name']} ({alert['severity'].upper()}): {alert['message']}")
        else:
    ...
    if report_data and args.mode in ["analysis", "reporting", "all"]:
        if isinstance(report_data, dict) and report_data.get("status") == "failed":
            ...
            print("Report output/path:", report_data) # This could be a file path or report object
